...
const copyFile = (srcPath, destPath) => {
  ...
  if (fs.existsSync(destPath) && !force) {
    console.log(
      `  ${yellow('skip')}  ${path.relative(TARGET_DIR, destPath)} ${dim('(already exists, use --force to overwrite)')}`,
    );
    return 'skipped';
  }
...
const copyDirectory = (srcDir, destDir) => {
  ...
  if (fs.existsSync(srcPath)) {
    ...
    for (const file of mdFiles) {
      const result = copyFile(path.join(srcPath, file), path.join(TARGET_DIR, destDir, file));
      if (result === 'copied') {
        copied++;
      } else {
        skipped++;
...
const init = () => {
  ...
  for (const file of FILES) {
    const result = copyFile(path.join(PACKAGE_ROOT, file.src), path.join(TARGET_DIR, file.dest));
    if (result === 'copied') {
      copied++;
    } else {
      skipped++;
